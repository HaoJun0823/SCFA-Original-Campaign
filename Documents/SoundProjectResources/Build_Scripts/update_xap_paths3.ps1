...
# 旧提取目录映射
$oldDirs = @{
    AmbientTest = "$baseDir\Ambient"
    FMV_BG = "$baseDir\FMV"
    Op_Briefing = "$baseDir\OP_Briefing"
    Tutorial_SE = "$baseDir\Tutorial_SE"
    Music = "$baseDir\Music\Music_Final\Looped"
}
...
foreach ($bankName in $oldDirs.Keys) {
    ...
    foreach ($oldName in $oldHashes.Keys) {
        ...
        $match = $null
        foreach ($entry in $pcmHashes.GetEnumerator()) {
            if ($entry.Value -eq $oldHash) {
                $match = $entry.Key
                break
            }
        }
        if ($match) {
            $mapping[$oldName] = $match
        } else {
